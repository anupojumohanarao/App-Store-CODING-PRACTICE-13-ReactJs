...
class AppStore extends Component {
  ...
  onChangeSearch = event => {
    this.setState({mainSearchInput: event.target.value})
  }

  getMainApps = searchingApps => {
    const {activeId} = this.state
    const filterProjects = searchingApps.filter(
  ...
  getTotalResults = () => {
    ...
    const mainSearchResults = appsList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(mainSearchInput.toLowerCase()),
    )
  ...
  render() {
    const {mainSearchInput, activeId} = this.state
    const mainSearchResults = this.getTotalResults()
    const filterProjects = this.getMainApps(mainSearchResults)

    return (
      <div className="main-app-store-container">
        <div className="main-container">
          ...
          <ul className="main-tab-list-container">
            {tabsList.map(eachTab => (
              <TabItem
                key={eachTab.tabId}
                mainTabDetails={eachTab}
          ...
          <ul className="main-app-details-container">
            {filterProjects.map(eachApp => (
              <AppItem key={eachApp.appId} mainProjectDetails={eachApp} />
            ))}
          </ul>
        </div>
      </div>
